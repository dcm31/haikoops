...
# opening syllable text file
 ...
 for line in response:
  ...
  if tweetDict.has_key("text"):
   ...
   mock = tweet
   words = tweet.split()
   for word in words: #iteratiing though every word in thing
    if not syl_dict.has_key(word):
     ...
     syl += syl_dict[word]
     print syl
     if syl == 5: # ensures that it is truly a haiku on each line
      five = 1
     if syl == 12:
      twelve = 1 
   if mock == tweet and syl == 17 and five == 1 and twelve == 1: 
    print tweet
